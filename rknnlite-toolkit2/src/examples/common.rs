...
pub enum RknnCoreMask {
    Auto = rknpu2_sys::_rknn_core_mask_RKNN_NPU_CORE_AUTO,
    Npu0 = rknpu2_sys::_rknn_core_mask_RKNN_NPU_CORE_0,
    Npu1 = rknpu2_sys::_rknn_core_mask_RKNN_NPU_CORE_1,
    Npu2 = rknpu2_sys::_rknn_core_mask_RKNN_NPU_CORE_2,
    Npu0_1 = rknpu2_sys::_rknn_core_mask_RKNN_NPU_CORE_0_1,
    Npu0_1_2 = rknpu2_sys::_rknn_core_mask_RKNN_NPU_CORE_0_1_2,
    All = rknpu2_sys::_rknn_core_mask_RKNN_NPU_CORE_ALL,
    Undefined = rknpu2_sys::_rknn_core_mask_RKNN_NPU_CORE_UNDEFINED,
}
...
impl RKNNContext {
    pub fn load_model(model_path: &str) -> Result<Self> {
        ...
        call_rknpu2!(rknn_query(
            ctx,
            rknpu2_sys::_rknn_query_cmd_RKNN_QUERY_IN_OUT_NUM,
            io_num_ptr,
    ...
    pub fn get_sdk_version(&self) -> Result<SdkVersion> {
        ...
        call_rknpu2!(rknn_query(
            self.ctx,
            rknpu2_sys::_rknn_query_cmd_RKNN_QUERY_SDK_VERSION,
            sdk_ver_ptr,
    ...
    pub fn get_input_attrs(&self) -> Result<Vec<rknn_tensor_attr>> {
        self.get_attrs(
            self.n_input,
            rknpu2_sys::_rknn_query_cmd_RKNN_QUERY_INPUT_ATTR,
        )
    }
    ...
    pub fn get_output_attrs(&self) -> Result<Vec<rknn_tensor_attr>> {
        self.get_attrs(
            self.n_output,
            rknpu2_sys::_rknn_query_cmd_RKNN_QUERY_OUTPUT_ATTR,
        )
    }
    ...
    pub fn get_input_range(&self) -> Result<Vec<rknn_input_range>> {
        ...
        for i in 0..self.n_input {
            ...
            call_rknpu2!(rknn_query(
                self.ctx,
                rknpu2_sys::_rknn_query_cmd_RKNN_QUERY_INPUT_DYNAMIC_RANGE,
                ranges_ptr,
    ...
    pub fn set_input_shapes(&self, shapes: &mut Vec<rknn_tensor_attr>) -> Result<()> {
        call_rknpu2!(rknn_set_input_shapes(
            self.ctx,
            self.n_input,
            shapes.as_mut_ptr()
        ))?;
    ...
    pub fn set_core_mask(&self, core_mask: &RknnCoreMask) -> Result<()> {
        call_rknpu2!(rknn_set_core_mask(self.ctx, *core_mask as u32))?;
        Ok(())
    }
    ...
    pub(crate) fn set_inputs(
        ...
        let mut inputs: Vec<rknn_input> = resized_images
            ...
            .map(|(i, img)| {
                let height =
                    if input_attrs[i].fmt == rknpu2_sys::_rknn_tensor_format_RKNN_TENSOR_NHWC {
                ...
                rknn_input {
                    index: i as u32,
                    pass_through: 0,
                    fmt: rknpu2_sys::_rknn_tensor_format_RKNN_TENSOR_NHWC,
                    type_: rknpu2_sys::_rknn_tensor_type_RKNN_TENSOR_UINT8,
                    buf: buf.as_mut_ptr() as *mut ::std::os::raw::c_void,
                    size: buf.len() as u32,
                }
            })
            .collect();
        call_rknpu2!(rknn_inputs_set(self.ctx, self.n_input, inputs.as_mut_ptr()))?;
        Ok(())
    }
    ...
    fn _release_outputs(&self, mut outputs: Vec<rknn_output>) -> Result<()> {
        call_rknpu2!(rknn_outputs_release(
            self.ctx,
            self.n_output,
            outputs.as_mut_ptr()
        ))?;
    ...
    pub fn run(&self) -> Result<()> {
        call_rknpu2!(rknn_run(self.ctx, std::ptr::null_mut()))?;
        Ok(())
    }
}
