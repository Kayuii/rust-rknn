...
const HEADER: &'static str = "headers/wrapper.h";
...
fn main() {
    ...
    match generate_wrapper_h(HEADER) {
        Ok(_) => {
            let wrapper_path = Path::new(HEADER);
            let file = OpenOptions::new()
                .read(true)
                .write(true)
                .create(true)
                .open(wrapper_path)
                .expect("Failed to open wrapper.h");
            ...
            for f in LIBS.iter().chain(INCLUDES.iter()) {
                ...
                match download_image(&url, &f.dst) {
                    Ok(_) => {
                        if f.dst.ends_with(".h") {
                            if !lines.iter().any(|line| line.contains(&f.dst)) {
                                lines.push(format!("#include <{}>", &f.dst));
                            }
                        }
                    },
            ...
            let mut writer = OpenOptions::new()
                ...
                .truncate(true)
                .open(wrapper_path)
                .expect("Failed to open wrapper.h for writing");

            for line in lines {
                writeln!(writer, "{}", line).expect("Failed to write to wrapper.h");
            }
    ...
    let headers: String = INCLUDES.iter().map(|i| i.dst).collect::<Vec<&str>>().join(" ");
...
fn generate_wrapper_h(header: &str) -> Result<(), std::io::Error> {
    let wrapper_path = Path::new(header);
    Ok(if !Path::exists(&wrapper_path) {
